-- Vioflow MAM 数据库迁移脚本：添加团队管理和权限系统
...
-- 确保团队成员属于正确的团队（可选约束，因为用户可能属于多个团队）
-- 注意：这个约束可能过于严格，如果用户可以在多个团队中，应该移除
-- 暂时注释掉，根据实际业务需求决定是否启用
/*
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint 
    WHERE conname = 'check_team_member_team_match'
  ) THEN
    ALTER TABLE team_members
    ADD CONSTRAINT check_team_member_team_match
    CHECK (
      EXISTS (
