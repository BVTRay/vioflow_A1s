    ...
    <div className="fixed inset-0 top-14 z-50 bg-zinc-950 flex animate-in fade-in duration-200">
      ...
      <div className="flex-1 flex flex-col relative bg-black/50">
        ...
        <div className={`h-24 ${theme.bg.primary} border-t ${theme.border.primary} px-8 flex flex-col justify-center gap-4`}>
             <div className={`w-full h-1.5 ${theme.bg.tertiary} rounded-full cursor-pointer group relative`}>
         ...
         <div className={`p-4 border-t ${theme.border.primary} ${theme.bg.secondary}`}>
             <div className="relative">
                 <input 
                    ...
                    className={`w-full ${theme.bg.primary} border ${theme.border.primary} rounded-lg pl-4 pr-10 py-3 text-sm ${theme.text.secondary} focus:ring-1 focus:ring-indigo-500 focus:border-indigo-500 focus:outline-none transition-all`}
                 />
                 <button 
...
const Comment: React.FC<{ user: string, time: string, text: string, timestamp: string, active?: boolean }> = ({ user, time, text, timestamp, active }) => (
    <div className={`flex gap-3 group ${active ? 'opacity-100' : 'opacity-70 hover:opacity-100'}`}>
        ...
        <div className="flex-1">
            ...
            <div className={`p-3 rounded-lg border text-sm leading-relaxed ${active ? 'bg-indigo-500/10 border-indigo-500/30 text-indigo-100' : `${theme.bg.tertiary}/50 ${theme.border.primary} ${theme.text.muted}`}`}>
                {text}
            </div>
